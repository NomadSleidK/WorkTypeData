...
template <typename T> void Ex1ComandControl(T *array, int arraySize, T type) // функция контроля команд для обработки
{
    ...
    do{ //цикл для получения и вызова команд
        command = Ex1ComandInput(); //ввод команды для обработки массива
        system("clear");
        DispleyBaseArray(array, arraySize, type);
        switch (command) {
            case 1:
...
template <typename T> T *InputMassiv (int arraySize, T type, char typeId) //функция ввода массива
{
...
{
    T *array = NULL;
    array = InputMassiv(arraySize, type, typeId);//вызов заполнеия массива
